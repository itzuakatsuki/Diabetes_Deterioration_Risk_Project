...
================================================================
本程式使用臨床模型的併發症風險分數與文獻年度成本，
在確定性版之上對每筆臨床紀錄進行 N 次餘生前瞻醫療成本模擬；
預設時界採生命表平均餘命，亦可透過 HORIZON_MODE 切換為固定年齡情境。
...
1. 併發症狀態不確定性：
   ...
   形成無併發症、僅大血管、僅小血管及兩者皆有四種情境。
2. 成本參數不確定性：
   各併發症狀態的年度成本依 Gamma 分布進行 PSA 抽樣；
   COST_CV 為成本參數的不確定性假設。

物價校正使用上海醫療保健類 CPI 逐年連乘，將 2001 年成本換算為
2024 年人民幣；目前不再對 CPI 倍數額外設定隨機分布。

成本點估計來源(已核對原文表 1):
  陳興寶等(2003)《中国糖尿病杂志》11(4):238-241,上海復旦大學公共衛生學院。
  其表中成本幣值年為 2001 年人民幣。
  程式使用上海醫療保健類 CPI 逐年連乘，將 2001 年成本校正至
  TARGET_YEAR 所指定的幣值年；目前 TARGET_YEAR = 2024。

逐年 CPI 被視為已公布的情境參數，因此 INFLATION_CV = 0，
不再對物價校正倍數另外設定 Gamma 分布。

資料包含重複回診紀錄。個人結果表保留全部紀錄，但母體總成本、
個人成本排序圖與成本集中度分析僅納入每位病患的首筆紀錄，
避免同一病患重複計入母體總額。
...
4. 風險來源限制:
   ...
   不是逐年新發併發症機率。目前將基準時點的併發症機率與狀態假設外推至整個成本時界是較強的簡化假設，
   但未建立逐年病程轉移模型，因此不宜解讀為已驗證的個人餘生成本預測。
...
6. 重複病患:
   臨床資料包含重複回診紀錄。個人結果表仍保留全部紀錄，
   但母體總成本、個人成本排序圖與成本集中度分析只納入每位病患的首筆紀錄，
   避免同一病患被重複計入母體總額。
