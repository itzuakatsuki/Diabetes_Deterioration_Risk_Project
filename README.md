...
臨床橫斷面分析：使用人口學、生活型態與生化檢驗特徵，進行大血管、小血管與任一併發症的關聯分類與風險分層。
CGM 時間序列分析：計算血糖控制指標、建立 Markov 血糖狀態轉移矩陣，並使用 LSTM 預測下一筆血糖值。
兩條路線後續整合為：
...
```
這些 Y 代表該次紀錄中目前是否已有併發症，不代表未來新發生併發症。
3.2 HbA1c 迴歸
X：其他臨床特徵；
排除：`HbA1c` 與 `GA`；
Y：連續型 `HbA1c` 數值。
...
但目前仍有兩項限制：
`make_windows()` 依序列索引建立視窗，未檢查 timestamp，因此視窗可能跨越缺測區段。
`model.fit()` 使用 `validation_split=0.15`，未明確設定 `shuffle=False`，因此不能宣稱內部 validation 完全保持時間順序。
...
15. 研究限制
    ---
臨床資料是橫斷面資料，不能直接推論未來發病或因果關係。
樣本約 100 位病患，模型效能與特徵重要性可能具有較大不確定性。
部分病患有重複紀錄，必須使用 GroupKFold 或病患層級去重。
部分生化變數缺失比例較高，中位數插補仍可能造成偏差。
分群為探索性分析，不代表已驗證的臨床亞型。
四象限以樣本中位數切分，不是臨床標準門檻。
母體 AGP 與 Markov matrix 以監測事件合併，監測時間較長者可能具有較高權重。
母體 Markov 目前接受不超過 30 分鐘的轉移，尚未嚴格限制為 14–16 分鐘。
風險引擎的個人 Markov matrix 目前沒有使用 timestamp 排除跨缺測轉移。
LSTM 視窗可能跨越時間缺口。
LSTM 內部 validation 未明確設定 `shuffle=False`。
單一病患 LSTM 結果不能直接推論至整體族群。
尚未使用外部資料進行驗證。
`requirements.txt` 尚未完整建立。
輸入與輸出路徑仍為 legacy relative paths。
成本 CPI 倍數仍為 placeholder。
成本模型可能重複計算同一病患的多次紀錄。
成本模型假設 Macro 與 Micro 在給定 X 後可分開抽樣。
成本模型沒有逐年病程進展與死亡模型。
成本輸出目前只能視為探索性情境分析。
